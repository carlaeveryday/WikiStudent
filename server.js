...
async function avisarAdelantosRanking(userId, username, oldPoints, newPoints) {
  if (!supabaseAdmin) {
    console.warn('[Ranking] supabaseAdmin no disponible — no se pueden avisar adelantos (falta SUPABASE_SERVICE_ROLE_KEY en el .env).');
    return;
  }
  try {
    // Usuarios que tenían puntos >= a los tuyos ANTES (incluye empates: si
    // estabais empatados, seguían "por delante o igual" hasta este momento)
    // y AHORA tienen igual o menos que tú: a esos los acabas de adelantar.
    // OJO: antes esto usaba .gt() en vez de .gte(), así que dos cuentas
    // empatadas (típico al probar con cuentas nuevas a 0 puntos) nunca se
    // detectaban como "adelantadas" — quedaba fuera del rango por 1 punto.
    const { data: adelantados, error } = await supabaseAdmin
      .from('profiles')
      .select('id, username, notification_token, notification_prefs')
      .gte('points', oldPoints)
      .lte('points', newPoints)
      .neq('id', userId)
      .not('notification_token', 'is', null);
    if (error) throw error;

    console.log(`[Ranking] ${username} pasó de ${oldPoints} a ${newPoints} pts. Candidatos adelantados: ${adelantados?.length ?? 0}.`);

    for (const u of adelantados || []) {
      if (!pushPermitido(u.notification_prefs, 'ranking')) {
        console.log(`  ⏭️  ${u.username} tiene "Actualización de ranking" desactivada, se salta.`);
        continue;
      }
      ...
      if (ok) console.log(`  ✅ Aviso de ranking enviado a ${u.username}`);
    }
  } catch (err) {
